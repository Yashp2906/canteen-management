...
{/* CATEGORY FILTER */}
<div className="max-w-6xl mx-auto px-6 mt-8 overflow-x-auto scrollbar-hide">
  <div className="flex gap-3 w-max">
    
    {/* ALL BUTTON */}
    <button
      onClick={() => setActiveCategory('all')}
      className={`px-5 py-2.5 rounded-full text-sm font-bold uppercase tracking-widest transition-all border ${
        activeCategory === 'all'
    ...
    {categories.map((cat) => (
      <button
        ...
        className={`px-5 py-2.5 rounded-full text-sm font-bold uppercase tracking-widest transition-all border whitespace-nowrap ${
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredFoods.map((food) => {
            ...
            return (
              <div
                key={food.id}
                className={`group relative rounded-[2.5rem] bg-[#0F172A]/40 border border-white/5 overflow-hidden transition-all duration-500 hover:bg-[#0F172A]/80 hover:border-orange-500/30 ${
                  food.status === 'unavailable' ? 'grayscale opacity-60' : ''
                }`}
              >
                <div className="relative h-52 overflow-hidden">
                  {food.image_url ? (
                    <img src={food.image_url} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-110" />
                  ) : (
